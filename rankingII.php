			...
			foreach ($con as $val) { 
			   ...
			   //$bgcolor = ($aux % 2 === 0) ? '#FCFCFE' : '#e8ebec';
			   	if($aux<4)
				{
					if($val->url != "")
			  		{
						...
						.$val->url .'.jpg" width="119" height="119" style=""/>';
					 }
      		 		else
			  		{
					...
					echo '<p style="position: absolute;text-align: center;width: 115px;margin-left:5px;bottom: 23px;overflow: hidden;height: 15px; text-transform:capitalize; font-size: 12px;"><b>'.utf8_encode($val->nombre).'</b></p><p style="position: absolute;text-align: center;width: 100%;bottom: 0px;overflow: hidden;height: 18px;font-size: 12px;">'.$val->times.'</p></div>';
				}
				else{
					if($val->url != "")
			  		{
						echo '<div style="width: 125px;margin: 20px 30px 20px 20px;position: relative; float: left;"><img style="border: 5px solid #fff;border-bottom: 45px solid #fff;-webkit-box-shadow: 1px 4px 12px rgba(0, 0, 0, 1); -moz-box-shadow: 1px 4px 12px rgba(0, 0, 0, 1);box-shadow: 1px 4px 12px rgba(0, 0, 0, 1);" src="https://hooptap.s3.amazonaws.com/userprofiles/'
						.$val->url .'.jpg" width="119" height="119" style=""/>';
					 }
      		 		else
			  		{
