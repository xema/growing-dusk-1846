...
<script>(function(d, s, id) {
 ...
 <div>
	...
	<div id="ranking" style="background:url(https://growing-dusk-1846.herokuapp.com/images/fondo_ramon.jpg) 0 0 repeat-y; width:700px; padding: 0 38px 20px 46px; ">
		<?php 
			$contents = file_get_contents('http://prod.hooptap.com/preview/fb/ranking/list_buzz.php?reto_id=145');
			...
			foreach ($con->usuarios as $val) { 
			   ...
			   //$bgcolor = ($aux % 2 === 0) ? '#FCFCFE' : '#e8ebec';
			   	if($aux<4)
				{
					...
					echo '<p style="position: absolute;text-align: center;width: 115px;margin-left:5px;bottom: 23px;overflow: hidden;height: 15px; text-transform:capitalize; font-size: 13px;"><b>'.utf8_decode($val->username).'</b></p><p style="position: absolute;text-align: center;width: 100%;bottom: 5px;overflow: hidden;height: 18px;font-size: 12px;font-weight: 600; color: #4a4a4a;">Cadena de '.$val->marca.'</p></div>';
				}
				else{
					if($val->img != "")
			  		{
...
?>
	...
	<div id="pie"><img src="https://growing-dusk-1846.herokuapp.com/images/pie_ramon.jpg" width="784" height="9" /></div> 
